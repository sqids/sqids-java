...
public class Sqids {
    ...
    private Sqids(final Builder builder) {
        ...
        if (
            new HashSet<>(Arrays.asList(alphabet.split(""))).size() !=
            alphabetLength
        ) {
            throw new IllegalArgumentException(
        ...
        final List<String> alphabetChars = new ArrayList<>(
            Arrays.asList(alphabet.toLowerCase().split(""))
        );
        ...
        for (Long num : numbers) {
            if (num < 0) {
                throw new RuntimeException(
                    "Encoding supports numbers between 0 and " + Long.MAX_VALUE
                );
            }
        }
    ...
    private String encodeNumbers(
        ...
        for (int i = 0; i < numberSize; i++) {
            offset =
                ...
                this.alphabet.charAt(
                        (int) (numbers.get(i) % this.alphabetLength)
                    ) +
                i;
        }
        ...
        final StringBuilder alphabetB = new StringBuilder(
            this.alphabet.substring((int) offset)
        )
        ...
        for (String word : this.blockList) {
            if (word.length() <= lowercaseIdLength) {
                if (lowercaseIdLength <= 3 || word.length() <= 3) {
                    ...
                    if (
                        lowercaseId.startsWith(word) ||
                        lowercaseId.endsWith(word)
                    ) {
                        return true;
                    }
    ...
    public static final class Builder {
         ...
         */
        public static final Set<String> DEFAULT_BLOCK_LIST =
        ...
        public Builder blockList(final Set<String> blockList) {
            if (blockList != null) {
                this.blockList =
                    Collections.unmodifiableSet(new HashSet<>(blockList));
            }
            return this;
        }
